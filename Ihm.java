...
class verification {
    ...
    public verification(Etudiant etudiant) {
        if (!(Gala.estInscrit(etudiant))) { //n'est pas inscrit
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == btn1) {
            ...
            if (!(dateaujourdhui.isAfter(dateaujourdhui)) ) {//remplacer dateaujourdhui entre parenthese part date du gala -1mois
                if (Gala.getEtudiants_en_attente().contains(etudiant)){//Soit il a fait une demande et il est en attente du coup on lui montre sa place
                    AffnbPlace fen9 = new AffnbPlace(etudiant.nb_accompagnant + 1, etudiant);
                }
                else if(Gala.est_titulaire_reservation(etudiant)) {// soit il est a confirmer donc on lui affiche la frame pour confirmer
                    ConfirmReserv fen9 = new ConfirmReserv(etudiant);
                }
                else if (Gala.getEtudiants_inscrits().contains(etudiant)) {// soit il n'a pas fait de demande de reservation donc on met le frame demande de reservatiopn
...
class PlanTable implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(e.getSource()==btn2){
            frame.setVisible(false);
            MessageChoixauto fen13 = new MessageChoixauto(5,etudiant); // mettre methode choix automatique de table
...
class Plan implements ActionListener {
    ...
    JFrame frame = new JFrame("Gestion reservation");
    JLabel label = new JLabel("Retrouver le plan des tables sous forme n°table: place disponible "+ sautLigne  /*+ TableEtu.EntreySet*/, JLabel.CENTER);
...
class VerifAnnee {
    public VerifAnnee(Etudiant etu){
        if(etu.annee_formation == 5){ //si etu 5 annee
